...
- 每轮 3-5 个关键问题，最多 3 轮
- 如果描述已足够清晰完整，可跳过
- 如果确认需求没有问题，必须输出固定确认语：`需求已理解，现在可以输出到ALM，请确认！`
- 未拿到用户明确确认前，禁止执行任何 ALM 写操作（`update_story_mcp_tool` / `create_story_mcp_tool`）

**澄清输出要求（内部检查）**：
- 输出前必须形成“4 句结论”：业务目标、需求变化、关键需求点、验收口径
...
**1. 概要**
1-2 句话高度概括需求本质与本次变更边界。
不得逐条复述关键需求点细节。

**2. 关键需求点**
按需求条目独立列出，避免流程化命名（禁止“流程A/B/C”）：
- 需求点1：`调整项 + 业务口径/约束 + 结果`
- 需求点2：`调整项 + 业务口径/约束 + 结果`
...
- Key Description 分三段：概要、关键需求点、【UAT验证场景】（+按需参考数据）
- 概要与关键需求点之间必须空 1 行
- 关键需求点与【UAT验证场景】之间必须空 3 行
- 概要必须高层概括“目标+范围+结果”，不得复述关键需求点中的触发条件、取值来源、录入规则等条目细节
- 关键需求点应直接表达“改什么、按什么口径、达到什么结果”，不得使用流程A/B/C编号
- 不新增独立规则区块，避免与关键需求点重复
...
若 Step 2 判定“需求已清晰”，此阶段**不输出改写的具体内容**，仅输出以下固定确认语：

`需求已理解，现在可以输出到ALM，请确认！`

如用户要求查看草稿，再补充展示改写内容。

**必须等待用户明确确认后才执行更新。未确认前禁止调用 `update_story_mcp_tool`。**
...
<p><b>概要：</b>通过两个按时序执行的 LTS 定时任务，自动建立起租需求单 ↔ Tank 关联关系，并向提请人持续推送提醒。</p>
<p><br/></p>
<p><b>关键需求点：</b></p>
<ul>
  <li>需求点1（LTS1-建关联）：每日 02:00 扫描前一日更新附件的 Tank，识别表单名称、表单号与附件（可从附件命名末尾“_”后提取），更新 Tank CF“需求单号”并建立关联。</li>
  <li>需求点2（LTS2-推提醒）：每日 03:00 扫描由需求点1已建立关联且状态为 Draft/Approved/Rejected 的租超需求单，以“需求单号”匹配 Tank.CF“需求单号”并统计已关联数量与状态。</li>
  <li>需求点3（停止条件）：当某需求单已关联 Tank 总数 = 需求单要求 Tank 数量时，停止该需求单提醒。</li>
</ul>
<br/><br/><br/>
<h3>【UAT验证场景】</h3>
<table>...</table>
...
**更新规则**：
- 保留【Biz Background】/【Current Situation】，方案部分统一输出为【Enhancement & Solution】+【UAT验证场景】结构
...
- 必须先完成需求 clarify（或明确判定“已清晰”并输出固定确认语），再等待用户确认后才能更新 ALM
- 固定确认语必须为：`需求已理解，现在可以输出到ALM，请确认！`
- 保留背景，替代方案部分
...
如果用户提供的新需求不够清晰，先进行轻量澄清；如果确认需求没有问题，必须输出固定确认语：`需求已理解，现在可以输出到ALM，请确认！`

在拿到用户明确确认前，禁止调用 `create_story_mcp_tool`。
...
<p><b>关键需求点：</b></p>
<ul>
  <li>需求点1：调整项 + 业务口径/约束 + 结果</li>
  <li>需求点2：调整项 + 业务口径/约束 + 结果</li>
</ul>
<br/><br/><br/>
<h3>【UAT验证场景】</h3>
...
- 概要与关键需求点之间必须空 1 行
- 关键需求点与【UAT验证场景】之间必须空 3 行
- 概要只保留高层结论，不重复关键需求点条目内容
- 关键需求点按条目拆分表达，避免合并成长流程，且不得使用流程A/B/C编号
- 不新增独立规则区块，避免重复表达
...
当需求已清晰时，创建前**不输出新建内容草稿详情**，仅输出固定确认语：

`需求已理解，现在可以输出到ALM，请确认！`

如用户要求查看草稿，再补充展示 `STORY_NAME`、`STORY_DESCRIPTION` 摘要与字段清单。

**必须等待用户明确确认后才能调用 `create_story_mcp_tool`。未确认前禁止创建。**
...
- 不得在用户未确认时创建 Story
- 必须先完成需求 clarify（或明确判定“已清晰”并输出固定确认语），再进入创建确认
- 固定确认语必须为：`需求已理解，现在可以输出到ALM，请确认！`
