...
- **有则保留、无则省略**：原始描述中的报文示例、数据样本、格式说明、截图等，提取后放在 Key Description 末尾的「参考数据 & 截图」区域，不做删改；若原始描述未提供，则不输出该区域及标题
...
- 如果原始描述没有明确分区，则整体替换为：背景（精炼）+ Key Description（有参考数据则追加参考区块）
